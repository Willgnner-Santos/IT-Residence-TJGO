...
# # ===============================
# # 2. FUNÇÃO DE EMBEDDING
# # ===============================

# def embed_texts(texts, model_name="gemini-embedding-001", dimensionality=3072, task="RETRIEVAL_DOCUMENT"):
#     model = TextEmbeddingModel.from_pretrained(model_name)
#     kwargs = {"output_dimensionality": dimensionality}
    
#     embeddings = []
#     for i, text in enumerate(tqdm(texts, desc="Gerando embeddings")):
#         try:
#             text_input = TextEmbeddingInput(text, task)
#             response = model.get_embeddings([text_input], **kwargs)
#             embeddings.append(response[0].values)
#         except Exception as e:
#             print(f"[{i+1}/{len(texts)}] Erro ao gerar embedding: {e}")
...
# def carregar_json_original(caminho):
#     with open(caminho, "r", encoding="utf-8") as f:
#         try:
#             dados = json.load(f)  # carrega uma lista de objetos de uma vez
...
# # ===============================
# # 4. SALVAR NOVO JSON COM EMBEDDINGS
# # ===============================

# def salvar_json_embeddado(dados, embeddings, caminho_saida):
#     for i in range(len(dados)):
#         if i < len(embeddings):
#             dados[i]["embedding"] = embeddings[i]
#     with open(caminho_saida, "w", encoding="utf-8") as f:
#         json.dump(dados, f, ensure_ascii=False, indent=2)
#     print(f"Embeddings salvos em: {caminho_saida}")

#     # Salvar também como .npy apenas os vetores
#     np_embeddings = np.array([emb for emb in embeddings if emb is not None], dtype=np.float32)
#     np.save(r"C:\Users\willgnnerferreira\Documents\Modelos\Perfil-Conciliação-Xgboost-GCP\dados\Embeddings\vetoress_texto.npy", np_embeddings)
#     print("Embeddings salvos como arquivo .npy")

# # ===============================
# # 5. FUNÇÃO PARA CARREGAR JSON EMBEDDADO FUTURAMENTE
# # ===============================

# def carregar_json_com_embeddings(caminho):
#     with open(caminho, "r", encoding="utf-8") as f:
#         dados = json.load(f)

#     textos = [item.get("inteiro_teor_limpo", "") for item in dados if item.get("embedding")]
#     embeddings = [item["embedding"] for item in dados if item.get("embedding")]

#     print(f"{len(embeddings)} embeddings carregados.")
#     return dados, textos, embeddings
...
# if __name__ == "__main__":
#     caminho_entrada = r"C:\Users\willgnnerferreira\Documents\Modelos\Perfil-Conciliação-Xgboost-GCP\dados\dados_processos_cejusc_enriquecidos.json"
#     caminho_saida = r"C:\Users\willgnnerferreira\Documents\Modelos\Perfil-Conciliação-Xgboost-GCP\dados\Embeddings\dados_processos_cejusc_embeddeds.json"

#     dados = carregar_json_original(caminho_entrada)
#     textos = [item["inteiro_teor_limpo"] for item in dados if "inteiro_teor_limpo" in item and item["inteiro_teor_limpo"]]

#     print(f"Total de textos válidos: {len(textos)}")

#     embeddings = embed_texts(textos)
#     salvar_json_embeddado(dados, embeddings, caminho_saida)

#     # Teste de carregamento futuro
#     print("Testando carregamento futuro dos embeddings...")
#     _, _, _ = carregar_json_com_embeddings(caminho_saida)
